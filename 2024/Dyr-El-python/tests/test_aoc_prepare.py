...
def test_happily_read_from_url(
    day_2015_04, monkeypatch, env_proxy_dummies, env_session_abc123
):
...
def test_get_from_net_if_not_cached(
    day_2015_04, monkeypatch, tmp_path, env_session_abc123
):
    """Test to read from actual url"""
    path = tmp_path.absolute()
    monkeypatch.chdir(path=path)
